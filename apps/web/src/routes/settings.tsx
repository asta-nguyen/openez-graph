...
import { settingsEnvQueryOptions } from "../lib/queries";
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <Badge variant="secondary" className="text-xs px-1.5 py-0 h-4">stdio</Badge>
      </div>
      <p className="text-xs text-muted-foreground mb-3">Add to <InlineCode>{configPath}</InlineCode></p>
...
function SettingsPage() {
  ...
  return (
    <div className="page container mx-auto px-4 sm:px-6">
      <div>
        <h1>Settings</h1>
        <p className="text-muted-foreground text-sm">Environment-backed provider configuration.</p>
      </div>
      ...
      <Card>
        <CardHeader><CardTitle>MCP Server Setup</CardTitle></CardHeader>
        <CardContent className="flex flex-col gap-4">
          <McpProviderCard name="Claude Code" configPath="~/.claude/settings.json" config={claudeCodeConfig} />
          <McpProviderCard name="Claude Desktop" configPath="claude_desktop_config.json" config={claudeDesktopConfig} />
          <McpProviderCard name="Cline / VS Code" configPath="VS Code settings or Cline MCP config" config={clineConfig} />
          <McpProviderCard name="OpenCode" configPath="~/.config/opencode/opencode.json" config={openCodeConfig} />
        </CardContent>
      </Card>
    </div>
  );
