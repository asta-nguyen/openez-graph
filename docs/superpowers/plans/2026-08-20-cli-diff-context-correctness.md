...
- [x] **Step 1: Add failing mixed-state tests.** Create a temporary repository with one staged edit and one unstaged edit. Assert the default report contains both files, the staged report contains only the staged file, and ref + staged fails.
- [x] **Step 2: Run bun test tests/diff-context.test.ts tests/mcp-tools.test.ts; confirm the default mixed-state test fails because the current code invokes plain git diff.**
- [x] **Step 3: Implement one core Git-argument decision table:**
...
- [x] **Step 4: Remove CLI-only divergence and make MCP return { error, ref, staged } through the existing jsonResponse() helper.**
...
- [x] **Step 1:** Restore the Umami ID from origin/main; do not touch surrounding landing code.
- [x] **Step 2:** Document:
...
- [x] **Step 3:** Explain that MCP path/paths select registered workspace roots, not changed files. Explicitly document that untracked files are not included in this Git-diff phase.
- [x] **Step 4:** Commit only the focused documentation/revert:
...
- [x] **Step 1:** Add a failing CLI integration test with target.ts and caller.ts; index once, modify target.ts, invoke the CLI path, and assert the report contains the caller without manually building the graph.
- [x] **Step 2:** Run bun test tests/cli-diff.test.ts; confirm current CLI produces missing/empty graph context.
- [x] **Step 3:** Export the existing ensureGraphReady from packages/indexer/src/index.ts only if it is not already public; do not import private MCP helpers.
- [x] **Step 4:** Resolve the workspace using existing CLI registry rules, then add:
...
- [x] **Step 1:** Add failing tests for two unstaged hunks before/after one staged hunk; assert the same affected symbol and working-tree line range.
- [x] **Step 2:** Add parser tests for insertion, deletion, rename metadata, binary/mode-only no-hunk files, and empty diff.
- [x] **Step 3:** Implement an immutable, single-pass transform: apply deltas from earlier non-overlapping hunks in old-file coordinates; when a range overlaps a hunk, return the union in working-tree coordinates instead of shifting an already-shifted range again.
...
- [x] **Step 1:** Add tests for single/multi-workspace shape, maxTokens, invalid path, invalid ref, and ref + staged.
- [x] **Step 2:** Budget the serialized report with the existing token-fitting helper; omit/truncate formattedSummary before dropping structured symbols/files.
- [x] **Step 3:** Normalize errors through jsonResponse() while preserving the actionable message.
- [x] **Step 4:** Run and commit:
...
- [x] **Step 1:** Add failing tests for a deleted function and a historical rename; assert old-side symbol and line data.
- [x] **Step 2:** Add a typed Git-blob loader using the existing safe argument-array execution and a structured warning for a missing blob.
- [x] **Step 3:** Reuse the existing parser entrypoint for blob content.
- [x] **Step 4:** Map old ranges to old symbols and new ranges to current symbols; mark symbols added, modified, or deleted.
- [x] **Step 5:** Run bun test tests/diff-context.test.ts and document current-graph-only caller/callee semantics.
