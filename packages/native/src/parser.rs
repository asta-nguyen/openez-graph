...
fn normalize_c_call(value: &str) -> String {
  // Keep qualified names like obj->method and obj.method intact.
  value.to_string()
}
...
fn c_config() -> LanguageConfig {
  let mut call_ignores = HashSet::new();
  for s in [
    "if", "else", "while", "for", "switch", "case", "do", "return", "goto",
    "sizeof", "typeof", "_Alignof", "_Alignas", "break", "continue",
    "printk", "pr_err", "pr_warn", "pr_info", "pr_debug", "pr_emerg",
    "BUG_ON", "WARN_ON", "BUG", "WARN",
    "true", "false", "NULL", "void", "int", "char", "long", "short",
    "float", "double", "unsigned", "signed", "const", "static", "extern",
    "volatile", "register", "auto", "inline", "struct", "union", "enum",
    "typedef", "_Bool", "size_t", "ssize_t", "u8", "u16", "u32", "u64",
    "s8", "s16", "s32", "s64", "bool",
  ] {
    call_ignores.insert(s);
  }
  ...
  LanguageConfig {
    symbol_rules: vec![
      SymbolRule {
        ...
        establishes_context: true,
        is_exported: c_not_static,
        extract_name: Some(c_function_name),
        extract_context_name: None,
        receiver_field: None,
        normalize_receiver: None,
      },
      ...
      SymbolRule {
        ...
        name_field: "declarator",
        establishes_context: false,
        is_exported: c_not_underscore,
        extract_name: Some(c_typedef_name),
        extract_context_name: None,
        receiver_field: None,
        normalize_receiver: None,
      },
    ...
    import_node_types: vec!["preproc_include"],
    call_node_type: "call_expression",
    call_function_field: "function",
    call_ignores,
    normalize_call_name: normalize_c_call,
    context_node_types: vec!["function_definition"],
  }
}

fn c_not_underscore(name: &str, _text: &str) -> bool {
  !name.starts_with('_')
}

fn c_not_static(_name: &str, text: &str) -> bool {
  !text.starts_with("static")
}

fn c_find_identifier(n: &Node, source: &[u8]) -> Option<String> {
  if n.kind() == "identifier" {
    return Some(text_of(n, source));
  }
  for i in 0..n.named_child_count() {
    if let Some(child) = n.named_child(i) {
      if let Some(found) = c_find_identifier(&child, source) {
        return Some(found);
      }
    }
  }
...
fn c_function_name(node: &Node, source: &[u8]) -> Option<String> {
  ...
  c_find_identifier(&decl, source)
}

fn c_typedef_name(node: &Node, source: &[u8]) -> Option<String> {
  let decl = node.child_by_field_name("declarator")?;
  if decl.kind() == "type_identifier" || decl.kind() == "identifier" {
    return Some(text_of(&decl, source));
  }
  // For function pointer typedefs: typedef int (*name)(...), the name is a
  // type_identifier inside parenthesized_declarator — not the param identifiers.
  c_find_type_identifier(&decl, source)
}

fn c_find_type_identifier(n: &Node, source: &[u8]) -> Option<String> {
  if n.kind() == "type_identifier" {
    return Some(text_of(n, source));
  }
  for i in 0..n.named_child_count() {
    if let Some(child) = n.named_child(i) {
      if let Some(found) = c_find_type_identifier(&child, source) {
        return Some(found);
      }
    }
  }
...
/// Only treat struct/union/enum as a symbol if it has a body (definition).
/// Type references in function params (e.g. `struct foo *x`) don't have `{`.
fn c_is_definition(_name: &str, text: &str) -> bool {
  text.contains('{')
}
...
fn extract_imports(node: &Node, source: &[u8], language: &str) -> Vec<String> {
  ...
  match language {
    ...
    "c" => {
      // preproc_include — path field contains <foo.h> or "foo.h"
  ...
  while let Some(node) = stack.pop() {
    ...
    if let Some((_, rule)) = symbol_rule_map.iter().find(|(t, _)| *t == node_kind) {
      ...
      if let Some(name) = name {
        ...
        // For C struct/union/enum: skip type references (no body).
        if !exported && (node_kind == "struct_specifier" || node_kind == "union_specifier" || node_kind == "enum_specifier") {
          continue;
        }
