...
pub fn parse_code_native(language: String, content: String) -> Option<NativeParseResult> {
  ...
  if matches!(lang, "typescript" | "tsx" | "javascript" | "jsx") {
    return crate::oxc_parser::parse_ts_js(&content, lang);
  }
...
pub fn parse_code_batch(items: Vec<ParseBatchItem>) -> Vec<Option<NativeParseResult>> {
  items
    .par_iter()
    .map(|item| {
      let lang_str = item.language.as_str();
      if matches!(lang_str, "typescript" | "tsx" | "javascript" | "jsx") {
        return crate::oxc_parser::parse_ts_js(&item.content, lang_str);
      }
