...
function extractCImports(node: Node): string[] {
  const paths: string[] = [];
...
export const cConfig: LanguageConfig = {
  ...
  symbolRules: [
    {
      ...
      extractName: (node) => {
        // declarator is a function_declarator node; name is in its declarator field
        const decl = node.childForFieldName("declarator");
        if (!decl) return null;
        // Navigate: function_declarator > identifier (or pointer_declarator > function_declarator > identifier)
        const findName = (n: Node): string | null => {
        ...
        return findName(decl);
      },
      establishesContext: true,
      isExported: (_name, node) => {
        // Check if static keyword is present in the function's storage class
        const text = node.text.slice(0, 30);
    ...
    },
    {
      ...
      extractName: (node) => {
        const decl = node.childForFieldName("declarator");
        if (!decl) return null;
        // type_definition > type_identifier (simple typedef) or > struct_specifier etc.
        // The declarator field holds the new name
        const findName = (n: Node): string | null => {
  ...
  importRules: [{ nodeType: "preproc_include", extract: extractCImports }],
  callRule: { nodeType: "call_expression", functionField: "function" },
  callIgnores: C_CALL_IGNORES,
  normalizeCallName: (value) => {
    const parts = value.split("->").filter(Boolean);
    const last = parts[parts.length - 1] ?? value;
    const dotParts = last.split(".");
    return dotParts[dotParts.length - 1] ?? last;
  },
  contextNodeTypes: new Set(["function_definition"]),
  contextNameField: "declarator",
};
